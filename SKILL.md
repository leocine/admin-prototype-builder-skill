...
- 原型数据必须使用明显虚构的名称和内容，并在交付说明中明确数据来源。不得为了标识原型而在业务界面中增加“原型数据”Badge、状态切换器或其他调试控件。除非用户明确要求，否则不得连接真实 API。
...
- 在源码中覆盖加载、空数据、失败、禁用和成功状态；需要独立检查时使用 `?prototypeState=<state>` 等非可见入口，默认页面必须保持正常业务界面。
- 运营、客服、审核和订单类列表默认采用紧凑密度：减少无意义 Card、压缩工具栏与表格行距，并根据可用宽度决定筛选项同排、换行或分组，不得把“同一行”写成固定布局。
- 需求包含明确的汇总指标时，在列表标题下、筛选区上方展示统计摘要卡片；没有指标口径时不得展示或自行编造统计数据。
- 列表分页必须同时说明当前页、总页数和总条数，并提供数字页码与前后翻页；数据规模较大时补充首页、尾页和跳页。
- 表格和详情中的缺失值统一使用“—”。
...
修复全部失败项。随后在浏览器中直接打开导出的文件，分别以约 1440px 桌面宽度和 430px 窄屏宽度检查。确认交互、表格溢出、文字换行和焦点可见性均正常。通过非可见状态入口分别检查加载、空数据、失败、禁用和成功状态，不得在默认业务界面中显示原型验收工具。
...
- 不得虚构生产事实、权限、指标或 API 结果；演示数据必须在源码或交付说明中标注为虚构数据，不得把标记放进默认业务界面。
- 不得在默认业务界面中显示“原型数据”、状态预览、设备切换或其他原型调试控件。
