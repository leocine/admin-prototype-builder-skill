...
```text
请使用 skill-installer 从 GitHub 私有仓库 leocine/admin-prototype-builder-skill 安装根目录的 admin-prototype-builder Skill，使用最新 Release。安装完成后提醒我开启一个新任务再使用。
```

安装完成后，开启一个新的 Codex 任务。

如果团队采用 Git 同步方式，也可以把仓库克隆到 `~/.codex/skills/admin-prototype-builder`。后续拉取 `main` 即可同步最新版；正式使用时以最新 GitHub Release 为版本基准。
