...
本 Skill 位于公开仓库根目录，无需登录 GitHub 即可安装。推荐通过 Git 克隆，以便后续完整同步 `assets/`、`references/` 和 `scripts/`。

新开一个 Codex 任务，把下面这段话直接发给 Codex：

```text
请把公开仓库 leocine/admin-prototype-builder-skill 安装到 ~/.codex/skills/admin-prototype-builder。如果目录不存在，使用 git clone https://github.com/leocine/admin-prototype-builder-skill.git ~/.codex/skills/admin-prototype-builder；如果目录已存在，先确认 origin 指向该仓库，再使用 git pull --ff-only 更新。完成后验证 SKILL.md、assets、references 和 scripts 都存在，并提醒我新开一个 Codex 任务。
```
